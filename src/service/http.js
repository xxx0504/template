...
// 响应拦截
HttpClient.interceptors.response.use(responseInterceptors
...
const http = {
    ...
    async post(url, params) {
        try {
            const res = await HttpClient.post(url, JSON.stringify(params)).catch((err) => {
                throw err.data;
            });
            return res.data;
        }
    ...
    downloadText: async function (urlUrl) {
        try {
    ...
    downloadFile: async function (urlUrl) {
        try {
